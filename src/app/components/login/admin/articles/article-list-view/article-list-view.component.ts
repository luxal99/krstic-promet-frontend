...
export class ArticleListViewComponent implements OnInit {
    ...
    getQuery() {
        if (this.behaviorService) {
            ...
            if (this.behaviorService.constructor.name === "WarehouseBehaviorService") {
                this.listOfArticle$ = this.listOfArticle$.pipe(map(value => value.filter((item: any) =>
                    item.idWarehouse.id === id)));
            } else if (this.behaviorService.constructor.name === "ArticleSubCategoryBehaviorService") {
                this.listOfArticle$ = this.listOfArticle$.pipe(map(value => value.filter((item: any) =>
                    item.idArticleSubCategory.id === id)));
            }
        }
    }
