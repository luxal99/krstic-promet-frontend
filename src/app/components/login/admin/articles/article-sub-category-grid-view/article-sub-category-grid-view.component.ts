...
export class ArticleSubCategoryGridViewComponent implements OnInit {
    ...
    listOfArticleSubCategories$: Observable<any> = this.articleSubCategory.select(state => state.articleSubCategory.list);
    @Input() tableConfig!: FormBuilderConfig;

    constructor(private articleSubCategory: Store<{ articleSubCategory: WarehouseState }>,
    ...
    filterArticleCategories() {
        ...
        this.listOfArticleSubCategories$ = this.listOfArticleSubCategories$.pipe(map(value => value.filter((item: any) =>
            item.idArticleCategory.id === id)));
    }
    ...
    loadArticleSubCategoryView(): void {
        ...
        articleListViewComponentComponentRef.instance.behaviorService = this.articleSubCategoryBehaviorService;
